...
> **Форма журнала:** `docs/` содержит основную документацию. Технический статус фиксируется в `journal.md`.  
> **Дополнение на корневом уровне:** Файл `PROJECT_UPDATE_STATUS.md` (в корне) содержит дополнительный снапшот — что сделано И что дальше по каждой задаче (помимо журнала).
...
Исторические документы, завершённые отчёты, legacy-материалы. **Не вести — только хранить и ссылаться.**

Включает:
- **Старые журналы** — `journal_2025.md` (сессии 1–39)
- **Отчёты об аудитах** → `archive/audit_reports/` — снапшоты проверок
- **Завершённые отчёты** — RAG-поиск, миграции, очистки
- **Кейсы и troubleshooting** — материалы об онбординге клиентов
- **Legacy specs** — старые техзадания (актуальные в `docs/specs/`)

👉 [Полная навигация архива](archive/README.md)
