...
3. Дефолтный параметр `tenant_id=1` в helper-функциях скрывал забывчивость

**Исправлено:**
- Добавлены `tenant_id` фильтры во все 7 SQL запросов в `_handle_search` (arkentiy.py L915-955)
- Добавлены `tenant_id` параметр и фильтр в `get_exact_time_orders` (database_pg.py L1257)
- Убран дефолт из `get_module_chats_for_city` → теперь явная ошибка если не передан (database_pg.py L720)
- Добавлен урок в `docs/Уроки_и_баги.md` о multi-tenant трэпах

**Результат:** 
- Все команды теперь корректно фильтруют по текущему тенанту
- `/поиск` и `/точные` Шабурова больше не возвращают заказы из Барнаула/Абакана
- Защита от ошибок: explicit error если забыли передать tenant_id
