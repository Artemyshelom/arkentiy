...
async def job_late_alerts() -> None:
    ...
    for branch_name, state in _states.items():
        ...
        for num, d in list(state.deliveries.items()):
            ...
            # Если заказ >30 мин опаздывает, но курьер не назначен и статус не продвинулся
            # ("Новая"/"Не подтверждена") — вероятно отменён или брошен. Не алертим.
            # Реальные опоздания всегда имеют курьера к этому моменту.
            if (overdue_min > 30
                    and not d.get("courier")
                    and d.get("status") in ("Новая", "Не подтверждена")):
                continue
