...
### 1. Дайджест — из эмодзи-сокращений в смысловые метрики
- **Было:** `⚠️ Абакан: 4🔴 2🟡 — 1×быстро, 4×отмена, 1×раньше` (непонятно кому, склад ума: события vs сумма)
- **Стало:** `⚠️ Абакан: 4 отмены (1 с готовкой) · 1 быстрая` (читаемо, смысловое, можно действовать)
- **Реализация:** `_format_digest` переописан: трёхуровневое склонение (отмена/отмены/отмен), скобки только для "с готовкой>0"

### 2. Отмены — честная классификация по риску списания
- **Проблема:** "3 отмен с готовкой (🍳 списания не вернуть)" — ввёл в заблуждение. Готовка началась ≠ списание произошло
- **Решение:** 3 уровня через `_classify_cancel(cancel_reason)`:
  - 🔴 `confirmed_writeoff`: в причине явно есть "списание" → деньги потеряны
  - ⚠️ `cooked_unclear`: готовился но причина неясна → надо проверить
  - ✅ `no_writeoff`: переносы/дублирование/тесты → продукт перекинули, не пропал
- **Нравится:** менеджеры уже сами маркируют реальные списания в причине, мы просто парсим их логику

### 3. Новый детектор — скидка + оплата бонусами одновременно
- **Что ловит:** 1-2 заказа в неделю где клиент получил и скидку и платил бонусами (редко, архаичная схема)
- **Как работает:** SELECT где discount_type NOT NULL И payment_type LIKE '%лояльности%' И status != 'отменена'
- **Страница детали:** новая `/format_discount_bonus_detail` с группировкой по филиалам и кнопкой
- **Callback:** `audit_detail:city:date:discount_bonus` обработан в `handle_audit_callback`

### 4. Убрано шумное "Итого: N событий"
- **Было:** блок "Требует внимания: 3 отмены" + строка "Итого: 14 событий" + сводка "Отмены: 10 · Быстрые: 4"
- **Логика:** если ниже написано что отмены, заказы, к-во — сложить 10+4 может любой. Строка шума
- **Стало:** убрана строка, сводка компактнее

**Коммит:** `git commit -m "refactor: audit UX — readable digest, three-level cancel classification, discount+bonus page"`

---

## 2026-03-13: ✅ Комментарий клиента в карточке + алертах + /опоздания

**Сессия:** Добавлена видимость `comment` из `orders_raw` в три места (30 мин).

### 1. Карточка поиска (`_format_order_card`)
- **Было:** состав, статус, цена, время, адрес — но комментарий клиента пропускался
- **Стало:** добавлена строка `💬 {comment}` в конец перед return (если не пусто)
- **Пример:** "Без лука, пожалуйста, домофон не работает"

### 2. Алерты опозданий (`late_alerts.py`)
- **Было:** письмо про опозданбе, но без комментария заказчика
- **Стало:** `comment_line = f"💬 {comment}"` добавлена между курьером и тегом клиента (если есть)
- **Почему не DB-запрос:** комментарий уже есть в `d.get("comment")` (iiko Events хранит при deliveryOrderCreated/Updated)

### 3. `/опоздания` сводка (`_handle_late`)
- **Было:** только ком-мандный список без комментариев
- **Стало:** добавлено `"comment": d.get("comment")` в `results.append()`, потом в форматировании `💬 {comment}` между курьером и адресом

**Хитрость:** не дублируем SELECT, расширяем `_states.deliveries` структуру. Комментарий там уже есть.

**Коммит:** `git commit -m "feat: display order comment in search card, late alerts, /опоздания page"`

---

## 2026-03-13: ✅ Массовый фиксинг 6 критичных багов + 1 фича (payment_types)

**Сессия:** 8 ТЗ за один проход. Причина: tuple-ключи в `_states` при добавлении мультитенанта + timezone мисматч UTC+3/UTC+7 + забытый tenant_id.

### 1. Fix late_alerts.py — алерты опозданий не отправляются
- **Проблема:** Ключ `_states` изменился `str → tuple[int, str]`, но late_alerts.py не обновлён → кортеж вместо строки → фильтры не пасуют → все ветки пропускаются
- **Фикс:** Распаковка `for (tid, branch_name), state in _states.items()`
- **Результат:** Алерты отправляются во все города ✅

### 2. Fix get_realtime_fot() — ФОТ не учитывает закрытые смены
- **Проблема:** Где `clock_out IS NULL` — только активные смены, за день могли работать 7, в отчёте 4
- **Фикс:** `COALESCE(clock_out, NOW()) - clock_in` для всех смен, фильтр по дате сегодня
- **Результат:** ФОТ учитывает всех поваров за день ✅

### 3. Fix avg_*_current_min — прочерки вместо метрик
- **Проблема:** Контейнер UTC+3, iiko сохраняет UTC+7 → минусовые дельты → None
- **Фикс:** `now = datetime.now(timezone.utc) + timedelta(hours=7)` в трёх свойствах
- **Результат:** Метрики показываются: `(14 мин)`, `(28 мин)` ✅

### 4. Fix fot_pipeline — алерты про ставки во все чаты
- **Проблема:** no_rate уведомления шли в чаты опозданий (менеджерам это не нужно)
- **Фикс:** Собрать все в одно сообщение, отправить только Артемию (255968113)
- **Результат:** Чистые чаты, владелец информирован ✅

### 5. Fix _handle_late() — /опоздания всегда говорит нет опозданий
- **Проблема:** tuple-ключ не распаковывается (как в #1) + DATE_PATTERNS не содержит сегодня/today
- **Фикс:** Распаковка + DATE_PATTERNS расширены
- **Результат:** `/опоздания` работает, `/опоздания сегодня` роутится в БД ✅

### 6. Fix get_available_branches() — RT не загружается для T3
- **Проблема:** `get_branches(tenant_id)` теряет tenant_id в кеше → дефолт=1 → ищет неправильный ключ в _states
- **Фикс:** Обогащение `[{**b, "tenant_id": tenant_id} for b in branches]` в else-блоке
- **Результат:** Канск и Зеленогорск видят RT ✅

### 7. Fix _parse_period() — даты вроде 9.03 не парсятся
- **Проблема:** `\d{2}` требует 2 цифры → 9.03 падает × фильтр города → город не найтись
- **Фикс:** `\d{1,2}` + `zfill(2)` при парсинге
- **Результат:** `/отчёт зелен 9.03` работает ✅

### 8. Feature payment_types — разбивка по способам оплаты
- **Добавлено:** _PAY_MAP маппинг (database_pg) + запросы (aggregate_orders_for_daily_stats, get_period_stats) + суммирование в city_aggregate + рендеринг в format_branch_report
- **Результат:** `/отчёт` показывает наличные/безнал/онлайн/бонусы с процентами ✅

**Коммит:** `git commit -m "fix: batch critical bugs (6) + payment_types feature (1) — 13 march"`

---

## 2026-03-12: ✅ UX: сводка /статус — 🔴 Касса закрыта (вариант Б, умный)

**Задача:** Когда кассовая смена закрыта и данных нет — показывать это сразу в сводке, а не скрывать за кнопкой.

**Решение:**
- **Сводка**: если `cash_shift_open == False` И `revenue == None` → показываем `🔴 Касса закрыта` в строке точки
- **Кнопка**: точка без кнопки (нечего смотреть детально)
- **Умный вариант**: если смена закрыта ИМА есть выручка (закрылась в конце дня) → кнопка остаётся (данные есть)

**Изменения:**
- `app/jobs/arkentiy.py`:
  - `_status_summary_line()` — проверка `cash_closed_no_data`, вывод `🔴 Касса закрыта` вместо `⚠️ нет данных`
  - `_build_status_summary()` — skip точек где смена закрыта и нет выручки при построении клавиатуры

**Файлы:** `app/jobs/arkentiy.py`  
**Коммит:** `303318f`
...
И т.д. (остальной контент без изменений)
...
**Проблема:**
Команда  не отображала строку ФОТ поваров, хотя данные в  были.
Причина:  в  не запрашивал / —
ФОТ читался только в автоматическом утреннем отчёте ().

**Решение:**
- Добавлены импорты ,  из 
- В  добавлен блок запроса ФОТ перед форматированием:
  - за один день → 
  - за период → 
- Формат вывода:  (за день с суммой, за период без)
- Всё в try/except — не ломает отчёт если данных нет
