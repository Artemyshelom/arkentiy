...
## 🟢 [ПРАКТИКА] Бэкфилл `orders_raw` для нового клиента через OLAP v2

**Контекст:** Онбординг Шабурова (tenant_id=3, март 2026). Нужно было заполнить `orders_raw` с 01.02.2026 — чтобы `/поиск` по телефону работал за весь период.

**Рабочий метод:** OLAP v2 с `groupByRowFields`:
```python
OLAP_GROUP_FIELDS = [
    "Delivery.Number",
    "Department",
    "Delivery.CustomerPhone",   # ← телефон клиента (работает в OLAP v2)
    "Delivery.CancelCause",
    "Delivery.ActualTime",
    "Delivery.Address",
    "Delivery.ServiceType",
    # OpenDate — НЕ ДОБАВЛЯТЬ (см. ниже)
]
```

**Эндпоинт:** `GET /api/v2/reports/olap?key={TOKEN}` с JSON-телом:
```json
{
    "reportType": "DELIVERY",
    "groupByRowFields": [...],
    "aggregateFields": [],
    "filters": {"openDate": {"filterType": "DateRange", "periodType": "CUSTOM", "from": "2026-02-01", "to": "2026-02-01"}}
}
```

**Скрипт-шаблон:** `app/onboarding/backfill_orders_shaburov.py` — взять за основу для нового клиента, поменять:
- `TENANT_ID`
- `iiko_credentials` в БД (уже есть если онбординг сделан)
- `SKIP_CITIES` (если у клиента сервер не отвечает)

**Запуск:**
```bash
# На VPS, внутри контейнера
docker compose exec app python -m app.onboarding.backfill_orders_shaburov 2>&1 | tee /opt/ebidoebi/logs/backfill_orders_shaburov.log
```

**Возобновление:** прогресс хранится в `/app/data/backfill_orders_shaburov_progress.json`. Если нужно перезапустить — сначала `pkill -f backfill_orders_shaburov`, потом удалить файл прогресса.
