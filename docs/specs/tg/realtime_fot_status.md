...
💼 ФОТ поваров: ~9.6% от выручки (3 чел · 4.2ч)
```

**Правила отображения:**
- Строка `💼 ФОТ поваров` — после `👥 На смене`, только если есть активные смены поваров
- Тильда `~` — расчётное значение, не финальное
- Кассовая смена: `🟢 открыта` / `🔴 закрыта` — перемещена под строку чеков
- Процент считается от текущей выручки на момент запроса
- Если поваров на смене нет — строку ФОТ не показывать
...
```python
# 1. Кассовая смена — переместить ПОСЛЕ строки чеков (было после себестоимости):
#    Найти блок cash_shift_open и переместить его вывод сразу после 🧾 Чеков

# 2. ФОТ поваров — после 👥 На смене:
if not db_fallback and (cooks is not None and cooks > 0) and revenue:
    try:
        rt_fot = await get_realtime_fot(name, tenant_id)
        if rt_fot and rt_fot['fot'] > 0 and revenue > 0:
