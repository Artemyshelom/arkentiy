...
- `TENANT_ID = N`
- `DATE_FROM = "YYYY-MM-DD"` (с какой даты бэкфилл)
- `SKIP_CITIES = {"ГородСПроблемнымОЛАП"}` (если OLAP таймаутит)
...
SELECT branch_name, COUNT(*), COUNT(*) FILTER (WHERE items != '') as with_items,
       COUNT(*) FILTER (WHERE courier != '') as with_courier,
       COUNT(*) FILTER (WHERE planned_time != '') as with_planned,
       COUNT(*) FILTER (WHERE client_name != '') as with_client_name
