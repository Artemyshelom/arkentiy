...
> **Роль файла:** конкретные OLAP-поля для бэкфилла `orders_raw` — timing-поля (opened_at, cooked_time, ready_time, send_time, service_print_time), проверенные на реальных серверах.
> **Чем отличается от `docs/reference/iiko_bo_api.md`:** тот файл — полный справочник endpoint'ов и всех доступных полей; этот — только timing-поля для `orders_raw`, с конкретными результатами проверки по тенантам.
