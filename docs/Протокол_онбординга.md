...
Для нового клиента изменить в скрипте:
- `TENANT_ID = N`
- `DATE_FROM = "YYYY-MM-DD"` (с какой даты бэкфилл, обычно начало подписки)
- `SKIP_CITIES = {"ГородСПроблемнымОЛАП"}` (если сервер таймаутит)

```bash
# 1. Убедиться что нет предыдущего запуска
ssh arkentiy "docker compose exec app pkill -f backfill_orders_shaburov || true"
ssh arkentiy "docker compose exec app rm -f /app/data/backfill_orders_shaburov_progress.json || true"

# 2. Запустить (логи в файл)
ssh arkentiy "cd /opt/ebidoebi && docker compose exec app python -m app.onboarding.backfill_orders_shaburov 2>&1 | tee /opt/ebidoebi/logs/backfill_orders_SLUG.log"

# 3. Мониторинг
ssh arkentiy "tail -f /opt/ebidoebi/logs/backfill_orders_SLUG.log"

# 4. Проверка итогов в БД
ssh arkentiy "docker compose exec postgres psql -U ebidoebi -d ebidoebi -c \"
SELECT branch_name, COUNT(*), MIN(order_date), MAX(order_date)
FROM orders_raw WHERE tenant_id = N GROUP BY branch_name ORDER BY 1;
\""
```

**⚠️ После бэкфилла — обязательно проверить `tenant_id`:**
```sql
-- Заказы нового клиента НЕ должны быть с tenant_id=1
SELECT tenant_id, branch_name, COUNT(*)
FROM orders_raw
WHERE branch_name IN ('Ветка_1', 'Ветка_2')
GROUP BY tenant_id, branch_name;
-- Если есть строки с tenant_id=1 — исправить:
UPDATE orders_raw SET tenant_id=N
WHERE branch_name IN ('Ветка_1','Ветка_2') AND tenant_id=1;
```

> **Почему может быть `tenant_id=1`:** Events API пишет заказы в `orders_raw` в реалтайме. Если подключение началось до правильной настройки маппинга — записи могут лечь с дефолтным `tenant_id=1`.
...
| `Delivery.Number` = null | `OpenDate` добавлен в `groupByRowFields` | Убрать `OpenDate` из group fields (брать из параметра фильтра) |
| Timeout 60s на OLAP v2 | Медленный сервер iiko или нет исторических данных | Добавить в `SKIP_CITIES`, уточнить у клиента |
