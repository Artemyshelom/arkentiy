...
try:
    ...
    # ... запись на диск при каждом вызове
# fallback: запись в /tmp/debug-3e913f.log
```
Этот код стал частью diff и прошёл в репозиторий без заметности. На продакшне функция вызывается сотни раз в сутки — писал на диск при каждом вызове.

**Удалено:** коммит `0e3a533` в ветке `fix/security-critical`.

**Lesson:**
- **Всегда читай diff перед коммитом**, даже если АИ сам делал 99% работы. Отладочный код выглядит как обычный `try/except pass` и легко проскальзывает.
- Перед пушем: `git diff main..HEAD -- app/` — быстрая проверка `import pathlib`, `open(`, `open("`, `/tmp/`, `.log`.
- Debug-код в `except: pass` блоке именно так размещается, чтобы не сломать основную логику при ошибке записи на диск.

---

## 🔴 [BUG] `backfill_orders_generic`: дата заказа берётся из week_start, а не OpenTime (март 2026)

**Проблема:** Клиентская статистика в Канске показывала 23 клиента при 210 чеках.

**Причина:** В `_upsert_deliveries` дата передавалась как `order_date` (= `week_start` чанка).
Заказы из чанка `2026-03-08..2026-03-10` попадали на дату `2026-03-08`.
Последний день диапазона (09.03) не попадал ни в один чанк из-за `includeHigh: false`.

**Вторая причина:** Прогресс-файл общий для всех серверов tenant'а.
Зеленогорск обрабатывался первым и помечал чанки `p1:2026-03-01`, `p1:2026-03-08` как выполненные — Канск их пропускал.

**Решение (коммит `359563c`):**
```python
# _aggregate_deliveries — извлекаем дату из OpenTime
order_date_str = opened_at[:10] if opened_at else None  # "2026-03-09T14:35:00" → "2026-03-09"
result[(branch, num)] = { ..., "order_date": order_date_str }

# _upsert_deliveries — используем реальную дату
real_date = date.fromisoformat(data["order_date"]) if data.get("order_date") else order_date

# ON CONFLICT — исправляем уже записанные строки
date = EXCLUDED.date,
```

**Lesson:** При записи UPSERT из недельных чанков ВСЕГДА брать дату из поля самого заказа (OpenTime), а не из параметра чанка. `includeHigh: false` гарантирует что последний день диапазона потеряется если дату не извлекать из данных.
